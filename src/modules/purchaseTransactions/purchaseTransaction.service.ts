...
export const updateStatus = async (
  ...
  if (input.depositAmount !== undefined)
    item.depositAmount = input.depositAmount;
...
export const release = async (
  ...
  await Listing.findByIdAndUpdate(pt.listing, {
    status: "sold",
    availabilityStatus: "sold",
  });
  ...
  await audit.record({
    actor: userId,
    actorRole: role,
    action: "purchase.escrow_released",
    targetType: "purchase_transaction",
    targetId: pt.id,
...
export const refund = async (
  ...
  await audit.record({
    actor: userId,
    actorRole: role,
    action: "purchase.escrow_refunded",
    targetType: "purchase_transaction",
    targetId: pt.id,
    metadata: { txHash: tx.txHash },
  });
...
export const resolveDispute = async (
  ...
  if (pt.escrow.state === "none") {
    ...
    const newStatus =
      input.decision === "refund" ? "cancelled" : "closing_review";
    const note =
    ...
    await audit.record({
      actor: userId,
      actorRole: role,
      action: "purchase.dispute_resolved",
      targetType: "purchase_transaction",
      targetId: pt.id,
