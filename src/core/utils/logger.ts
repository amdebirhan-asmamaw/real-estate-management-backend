...
export const logHttpRequest = (data: HttpLogData): void => {
  ...
  const methodColor =
    data.method === "GET"    ? green  :
    data.method === "POST"   ? "\x1b[34m" :  // Blue
    data.method === "PUT"    ? "\x1b[33m" :  // Yellow
    data.method === "PATCH"  ? "\x1b[35m" :  // Magenta
    data.method === "DELETE" ? "\x1b[31m" :  // Red
    reset;
  ...
  if (data.body && typeof data.body === "object" && Object.keys(data.body as object).length > 0) {
    lines.push(`  ${yellow}Body      ${reset}${JSON.stringify(redact(data.body), null, 2)}`);
  }
